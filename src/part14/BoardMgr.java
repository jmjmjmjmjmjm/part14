...
public class BoardMgr {
	...
	private static final String SAVEFOLDER = "C:/";
	...
	public BoardMgr() {
		try {
			pool = DBConnectionMgr.getInstance2();
		} catch (Exception e) {
			System.out.println("보드쪽Mgr안됨");
		}
	}

	// 게시물 리스트
	public Vector<BoardBean> getBoardList(String keyFiled, String keyWord, int start, int end) {
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				...
				sql = "select *FROM tblBoard where" + keyFiled + "like ? ";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			System.out.println("총 게시물수가 출력이안됨");
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
	...
	public void replyUpBoard(int ref, int pos) {
		try {
			...
			System.out.println("답변위 위치값 증가오류");
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
	}
	...
	public void downLoad(HttpServletRequest req, HttpServletResponse res, JspWriter out, PageContext pageContext) {
		...
		try {
			...
			if (strClient.indexOf("MSIE6.0") != -1) {
				...
				res.setHeader("Content-Dispositon", "filename = " + filename + ";");
			} else {
				res.setContentType("application/smnet;charset=UTF-8");
	...
	public void post1000() {
		try {
			...
			System.out.println("페이징 블럭 테스트 저장메소드 오류");
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
	}
