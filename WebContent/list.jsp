...
<%@page import="part14.BoardBean"%>
<%@page import="java.util.Vector"%>
<jsp:useBean id="bMgr" class="part14.BoardMgr" />
...
<link href="style.css" rel="stylesheet" type="texxt/css">
<script type="text/javascript">
	...
	function block(value) {
		document.readFrm.nowPage.vlaue =
<%=pagePerBlock%>
	* (value - 1) + 1;
		document.readFrm.submit();
	}
...
<body bgcolor="$FFFFCC">
		...
		<table align="center" width="80%" border="0" cellspacing="0"
			...
			<tr>
				<td align="center" colspan="2">
					...
					%>
					<table border="0" width="100%" cellpadding="2" cellsacing="0">
 ...
 int pageEnd = ((pageStart + pagePerBlock) < totalPage) ? (pageStart + pagePerBlock) : totalPage + 1;
 	//하단 페이지 끝 번호
 	if (totalPage != 0){
 		if (nowBlock > 1) {%> <a href="javascript:block('<%=nowBlock-1%>')">prev...</a>
					...
					<%if(pageStart==nowPage) { %><font color = "blue"><%} %>
					 [<%=pageStart %>]
						...
						<%}//for %>&nbsp;
						 <%if (totalBlock>nowBlock){ %> <a
					href="javascriipt:block('<%=nowBlock +1%>')">.....next</a> <%}%>&nbsp;<%} %>
					<!-- 페이징 및 블럭처리 끝 -->
				</td>
